...
namespace ToDoLL
{
    public class Book
    {
        ...
        { 
            ...
            public Book nodeWorkItem; // holds an object with real data
            ...
            public LinkedListNode(Book pWorkItem)
            {
        ...
        public void InsertAtFront(Book pWorkItem)
        {
        ...
        public void InsertInOrder(Book pWorkItem)
        {
            ...
            // this has to changd
            if (frontOfList == null || frontOfList.nodeWorkItem.Rating >= pWorkItem.Rating)  // watch the pointers to see what happens
            {
            ...
            else  // we have to traverse the LL and find where to add a new node
            {
                ...
                // this line changed
                while (current.node_next_pointer != null && current.node_next_pointer.nodeWorkItem.Rating < pWorkItem.Rating)
                {
        ...
        // write new method here: (could assume 0 is highest priority, or 1, either way works, as long as constants are consistent
        // if index is larger than the number of items on the list, we will default to just making it last
        // Method to add a node in order, based on a supplied Index

        public void InsertAt(Book pWorkItem, int index)
        {
            // if list is empty, or if the desired index is 1, then we want front to point to our new node.
            if (frontOfList == null || index == 1)
            {
                InsertAtFront(pWorkItem);  // so just call our insert at front, which already handles the zero items or exisitng items correctly
                return; // we are done  
            }

            LinkedListNode newNode = new LinkedListNode(pWorkItem); // create our new node to be added, and pass in the object
            LinkedListNode current = frontOfList; // set our object reference to equal the first item in the list
            int counter = 1; // keep track of where we are in the list.

            // This while statements job is to walk thru the LL looking for the right place to insert.
            // when it exits, it is then time to do the insert.  So it leaves when either it is at the bottom of the LL
            // or if it has walked down the LL to the location one BEFORE the new node should be inserted 
            // in which case it falls out of the while and the insert happens.
            while (current.node_next_pointer != null && counter < index - 1)
            {
                current = current.node_next_pointer;  // move the value of our pointer to walk down one step in the list
                counter++;  // bump our index count up as we just moved one step
            }
